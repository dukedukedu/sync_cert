...
if [[ "$PLATFORM" == "aws" ]]; then
  ...
  # az expects using the managed identity, 
  # use
  # az login --identity
  # at least once in the session to tell az to use the VM's identity
...
if $cert_changed; then
  ...
  if ! command -v nginx >/dev/null 2>&1; then
    log "[WARNING] Certificate installed or updated, but Nginx is not installed. Manual intervention required — please reload any services that use this certificate."
    notify_admin \
      "The SSL certificate was installed or updated successfully, but Nginx is not installed on this system.

Manual intervention is required — please reload any services that depend on this certificate to ensure the changes take effect.

This message was generated by the sync_cert.sh process on $HOSTNAME_FQDN." \
    "Certificate Updated – Manual Reload Required"
  elif nginx -t; then
